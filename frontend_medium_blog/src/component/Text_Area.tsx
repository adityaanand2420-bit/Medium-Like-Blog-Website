import { useCallback, useState, type ChangeEvent, type MouseEventHandler } from "react";
...
export function TextArea({ onchange, onclick }: fnType) {
  ...
  const [Content, setContent] = useState("");
   ...
   const HandlefilePicker = useCallback(async ()=>{
        ...
        input.type = "file";
        input.accept = "image/*" //It tells the browser to filter the file picker so the user can only select image files.
        input.multiple= true;
  ...
  const HandleFiles = async (files: File[]) => {
    ...
    try {
      for (const file of files) {
        ...
        const imghtml = `<img src="${url}" alt="${file.name}" width="125px" height="125px"/>`;
  ...
  return (
    <form className="h-full">
      <div className="w-full mb-4 border border-default-medium rounded-base bg-neutral-secondary-medium shadow-xs h-full max-h-4/5">
        ...
        <div className="px-4 py-2 bg-neutral-secondary-medium rounded-b-base h-full  max-h-4/5">
          <div
            id="editor"
            contentEditable="true"
            dangerouslySetInnerHTML={{ __html: Content }}
            className="block w-full px-3 text-sm text-heading bg-neutral-secondary-medium border-0 focus:ring-0 placeholder:text-body focus:outline-none h-full "
          ></div>
        </div>
      </div>
      <button
        type="button"
        className="text-white bg-gray-950 box-border border border-transparent hover:bg-gray-800 focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-lg text-sm px-4 py-2.5 focus:outline-none cursor-pointer"
        onClick={onclick}
      >
        Publish post
      </button>
